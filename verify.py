...
import hashlib, os, shutil, struct, subprocess, tempfile, time
...
def integrity():
    ...
    print(f'PASS integrity ({len(H)} certificates)')
...
def vc(A,b,M,c):
    p,u,v,row,k=c
    if p<=0 or row not in (0,5) or not 0<=k<=4 or u+v<=2:raise ValueError
    if fall(u,k)*fall(v,4-k)<=0:raise ValueError
    G=GF(A,b,0);B=BF(b,0)
...
def ve(A,M,s,row):
    if s<4 or row not in (0,5) or Q(s)*XMAX>A.lo:raise ValueError
...
def loadtail(p):
    d=p.read_bytes();o=0
    if len(d)<16 or d[:8]!=b'ALBTL01\0':raise ValueError('bad tail magic')
    o=8;r,n=struct.unpack_from('<II',d,o);o+=8
    if r!=1000 or n==0:raise ValueError('bad tail header')
    bs=[]
    for _ in range(n):
        if o+33>len(d):raise ValueError('truncated tail box')
        ln,ld,hn,hd=struct.unpack_from('<qqqq',d,o);o+=32
        if ld<=0 or hd<=0:raise ValueError('nonpositive endpoint denominator')
        m=d[o];o+=1;cs=[]
        if o+5*m+2>len(d):raise ValueError('truncated tail record')
        for _ in range(m):
            cap=tuple(d[o:o+5]);o+=5
            if cap[0]==0 or cap[3] not in (0,5) or cap[4]>4 or cap[1]+cap[2]<=2:raise ValueError('illegal cap record')
            cs.append(cap)
        s=d[o];row=d[o+1];o+=2
        if s<4 or row not in (0,5):raise ValueError('illegal final record')
        bs.append(Box(Q(ln,ld),Q(hn,hd),cs,s,row))
    if o!=len(d):raise ValueError('trailing tail bytes')
    return r,bs
