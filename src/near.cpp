...
struct Q{
    ...
    void norm(){if(d<0){d=-d;n=-n;}if(n==0){d=1;return;}i128 g=gcd128(n,d);n/=g;d/=g;}
};
...
static long long base_edges(int k,int n){
    ...
    if(n!=2*k-1)v=max(v,ceildiv(1LL*(k-1)*n+2LL*(k-3),2));
    return v;
}
...
int main(int ac,char**av){const char*path=ac>1?av[1]:"cert/near_residual.bin";init_bounds();ifstream f(path,ios::binary);char magic[8];f.read(magic,8);if(memcmp(magic,"ALBNR01",7)){cerr<<"magic\n";return 3;}uint32_t N;f.read((char*)&N,4);unordered_map<int,Cert> mp;mp.reserve(N*2);for(uint32_t i=0;i<N;i++){uint16_t r,c;uint8_t m;f.read((char*)&r,2);f.read((char*)&c,2);f.read((char*)&m,1);Cert z{r,c,{}};z.w.resize(m);for(auto&x:z.w){f.read((char*)&x.typ,1);f.read((char*)&x.a,2);f.read((char*)&x.b,2);f.read((char*)&x.num,8);f.read((char*)&x.den,8);}mp[r*1000+c]=move(z);}char tail;if(f.read(&tail,1)){cerr<<"trailing\n";return 3;}long long small=0,completion=0,routing=0,direct=0,lp=0;unordered_set<int>used;auto st=chrono::steady_clock::now();for(int r=19;r<=999;r++){int cm=(57*r-1)/250;for(int c=0;c<=cm;c++){if(c<=5){small++;continue;}if(completion_ok(r,c)){completion++;continue;}if(routing_ok(r,c)){routing++;continue;}Cfg cfg=config(r,c);if(cfg.total_low>cfg.total_high){direct++;continue;}int key=r*1000+c;auto it=mp.find(key);if(it==mp.end()||r>204||!check_cert(cfg,it->second)){cerr<<"FAIL r="<<r<<" c="<<c<<"\n";return 2;}used.insert(key);lp++;}}if(used.size()!=mp.size()){cerr<<"unused certs "<<mp.size()-used.size()<<"\n";return 2;}double sec=chrono::duration<double>(chrono::steady_clock::now()-st).count();cout<<"PASS near finite exact counts small="<<small<<" completion="<<completion<<" routing="<<routing<<" direct="<<direct<<" lp="<<lp<<" sec="<<sec<<"\n";}
